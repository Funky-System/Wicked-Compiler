...
mpc_parser_t *mpc_blank_nonl(void) { return mpc_expect(mpc_apply(mpc_whitespaces_nonl(), mpcf_free), "whitespace (no newline)"); }
...
mpc_parser_t *mpc_tok_nonl(mpc_parser_t *a) { return mpc_and(2, mpcf_fst, a, mpc_blank_nonl(), mpcf_dtor_null); }
...
static mpc_val_t *mpcaf_grammar_string(mpc_val_t *x, void *s) {
  ...
  mpc_parser_t *p = (st->flags & MPCA_LANG_WHITESPACE_SENSITIVE) ? mpc_string(y) :
                    (st->flags & MPCA_LANG_NEWLINE_SENSITIVE) ? mpc_tok_nonl(mpc_string(y)) : mpc_tok(mpc_string(y));
  free(y);
...
static mpc_val_t *mpcaf_grammar_char(mpc_val_t *x, void *s) {
  mpca_grammar_st_t *st = s;
  char *y = mpcf_unescape(x);
  mpc_parser_t *p = (st->flags & MPCA_LANG_WHITESPACE_SENSITIVE) ? mpc_char(y[0]) :
                    (st->flags & MPCA_LANG_NEWLINE_SENSITIVE) ? mpc_tok_nonl(mpc_char(y[0])) : mpc_tok(mpc_char(y[0]));
  free(y);
...
static mpc_val_t *mpcaf_grammar_regex(mpc_val_t *x, void *s) {
  ...
  mpc_parser_t *p = (st->flags & MPCA_LANG_WHITESPACE_SENSITIVE) ? mpc_re(y) :
                    (st->flags & MPCA_LANG_NEWLINE_SENSITIVE) ? mpc_tok_nonl(mpc_re(y)) : mpc_tok(mpc_re(y));
  free(y);
